...
def test_threshold(args):
    ...
    delay = 0.5 * ((rate/20)-1) / rate
    plt.style.use('ggplot')
    plt.title('Signal Envelope, Threshold = {}'.format(str(args.threshold)))
    plt.plot(wav[np.logical_not(mask)]-delay, color='r', label='remove')
    plt.plot(wav[mask]-delay, color='c', label='keep')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--fn', type=str, default='Joaquim_121',
